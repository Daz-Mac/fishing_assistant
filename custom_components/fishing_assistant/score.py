...
class FreshwaterFishingScorer(BaseScorer):
    ...
    def calculate_score(
        ...
        try:
            ...
            # Add forecast if available in weather_data (tolerant)
            if isinstance(weather_data, dict) and "forecast" in weather_data and isinstance(weather_data["forecast"], list):
    ...
    def _format_forecast(self, forecast_data: List[Dict[str, Any]], astro_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        for entry in forecast_data:
            try:
                ...
                normalized_scores = {key: round(value * 10, 1) for key, value in component_scores.items()}
    ...
    async def calculate_forecast(
        ...
        for weather_data in weather_forecast:
            try:
                ...
                # Add timestamp to result in ISO format
        ...
        try:
            if astro:
                ...
                if sunrise and sunset:
                    # Try to coerce to datetime if strings provided
                    sunrise_dt = self._coerce_datetime(sunrise) or None
                    sunset_dt = self._coerce_datetime(sunset) or None
                    if sunrise_dt and sunset_dt and current_time:
                        try:
                            ...
                            pass
            # Fallback to basic hour checks
            hour = getattr(current_time, "hour", datetime.now().hour)
            ...
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        return None
        # do not coerce numeric timestamps to avoid epoch/timezone ambiguity
        return None
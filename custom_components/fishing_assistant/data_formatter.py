...
This module provides functions to convert raw API data into standardized
...
def format_period_forecast(
    ...
    return PeriodForecast(
        ...
        weather=format_weather_data(weather),
    )
...
def format_daily_forecast(
    ...
    for period_name, period_data in periods.items():
        formatted_period = format_period_forecast(
...
def format_sensor_attributes(
    ...
    if forecast:
        for date_str, daily_data in forecast.items():
            formatted_forecast[date_str] = format_daily_forecast(
                date=date_str,
    ...
    return SensorAttributes(
        ...
        component_scores=format_component_scores(component_scores),
        weather=format_weather_data(weather),
        marine=format_marine_data(marine),
        tide=format_tide_data(tide),
        astro=format_astro_data(astro),
